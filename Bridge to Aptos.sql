...
select address, sum(amount) as amount from 
    ...
    where date_trunc('day',call_block_time)<=cast('2024-05-01' as TIMESTAMP)
    group by 1
    ...
    select address, sum(amount) as amount from (
        ...
        where _dstChainId=108 and date_trunc('day',evt_block_time)<=cast('2024-05-01' as TIMESTAMP))
    group by 1)
group by 1
),
op_apt_tx as (
select address, sum(volume) as volume from 
    (select address, sum(value) as volume from (
        ...
        where date_trunc('day',evt_block_time)<=cast('2024-05-01' as TIMESTAMP)
    )
    group by 1
    ...
    select address, sum(amount) as volume from (
        ...
        and date_trunc('day',et.evt_block_time)<=cast('2024-05-01' as TIMESTAMP)
    )
    group by 1)
group by 1
),
